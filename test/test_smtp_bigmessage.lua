...
function server_handlers:HAVE_DATA(reply, data, err)
    if data then
        reply.code = "250"
...
function client_handler(client)
    ...
    local send_data = client:send_data(string.rep("a", 26))
    assert(send_data.code == "552" and send_data.message == "5.3.4 message too big")
...
function server_ctx(host, port)
    ...
    handler.extensions:add("SIZE", "25")
    handler:handle()
end
-- }}}
